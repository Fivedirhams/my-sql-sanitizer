"""Title transformer — job title replacements via LLM."""
...
class TitleTransformer(BaseTransformer):
    """Replace job titles with realistic alternatives.
    
    Uses GlobalMappingRegistry for cross-table consistency.
    Falls back to deterministic shuffle pool instead of fake hashes.
    """
    ...
    def transform(self, value: Optional[str], table: str, column: str) -> Optional[str]:
        ...
        # Fallback: cycle within observed titles (deterministic rotation)
        ...
        # Absolute last resort: preserve original (safer than fake)
        return value
    ...
    def _load_mapping(
        ...
        # Register shuffle pool for unknown values
        ...
        if unseen:
            result = client.generate_title_mapping(field_key, unseen)
